...
} from "../../utils/slices/subcategoryProducts";
import { fetchVendorProducts } from "../../utils/slices/products";
...
const SubcategoryProductsPage: React.FC = () => {
    ...
    const { vendorProducts, status: vendorProductsStatus } = useSelector(
    ...
    const getSelectedProducts = (): SelectOptionType[] => {
        if (subcategoryProduct) {
            ...
            return subcategoryProduct.product_ids.map((product) => {
                const vproduct = vendorProducts.find(
                    (vp) => vp._id === product._id
                );
                return {
                    value: product._id,
